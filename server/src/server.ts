...
const DATA_DIR_INIT = path.join(
  __dirname,
  "../data/init"
);
const DATA_FILE_MISSIONS =
  "missions.json";
...
const loadMissions = async (): Promise<
  Mission[]
> =>
  await _g(async () => {
    return JSON.parse(
...
const main = async () => {
  ...
  const WEATHER_API_KEY =
    "11974b5f7a384a2e80d170259221306";
  ...
  app.get(
    ...
    async (
      ...
      try {
        ...
        const cityWeather =
          await weather.get(
            `https://api.weatherapi.com/v1/forecast.json?key=${WEATHER_API_KEY}&q=${req.query.city}&days=5&aqi=no&alerts=no`
          );
